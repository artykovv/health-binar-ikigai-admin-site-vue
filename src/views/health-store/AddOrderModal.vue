<template>
  <div v-if="visible" class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      <div class="relative bg-white dark:bg-[#3f3f47] rounded-[20px] shadow-xl max-w-4xl w-full p-6 animate-in fade-in zoom-in duration-200" @click.stop>
          ...
          <div class="flex flex-col h-full bg-gray-50 dark:bg-gray-800/50 rounded-[20px] p-6 border border-gray-100 dark:border-gray-700">
                ...
                <div class="flex-1 min-w-0 mr-3">
                  ...
                  <div class="flex flex-col mt-0.5">
                    <div class="flex gap-2 text-[10px] text-gray-400 line-through" v-if="totalDiscountPercent > 0">
                      ...
                      <span>{{ (item.price * 88).toLocaleString() }} сом</span>
                    </div>
                    <div class="flex gap-2 text-xs font-bold">
                      <span class="text-blue-600 dark:text-blue-400">${{ calculateDiscountedPrice(item.price).toFixed(2) }}</span>
                      <span class="text-emerald-600 dark:text-emerald-400">{{ (calculateDiscountedPrice(item.price) * 88).toLocaleString() }} сом</span>
                    </div>
                  </div>
                </div>
            ...
            <div class="pt-6 border-t border-gray-200 dark:border-gray-700 space-y-4">
              <div class="space-y-4">
                ...
                <div>
                  <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-wider">Способ оплаты</label>
                  <select v-model="selectedPaymentMethod" class="w-full py-2.5 px-4 rounded-xl border border-gray-200 bg-white dark:bg-gray-800 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all">
              ...
              <div class="space-y-2 pt-4">
                <div class="flex justify-between items-center text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <div class="flex gap-2 font-bold">
                    <span class="text-blue-600 dark:text-blue-400">${{ subtotal.toFixed(2) }}</span>
                    <span class="text-emerald-600 dark:text-emerald-400">{{ (subtotal * 88).toLocaleString() }} сом</span>
                  </div>
                </div>
                <div v-if="discountAmount > 0" class="flex justify-between items-center text-sm text-green-600 dark:text-green-400">
                  ...
                  <div class="flex gap-2 font-bold text-red-500">
                    ...
                    <span>-{{ (discountAmount * 88).toLocaleString() }} сом</span>
                  </div>
                </div>
                <div class="flex justify-between items-center text-lg font-bold text-gray-900 dark:text-white pt-2 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div class="flex gap-3">
                    ...
                    <span class="text-emerald-600 dark:text-emerald-400">{{ (total * 88).toLocaleString() }} сом</span>
                  </div>
                </div>
              </div>
