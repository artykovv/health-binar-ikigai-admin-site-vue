<template>
            ...
            <div class="space-y-2">
              ...
              >
                  <div class="flex-1 text-sm text-gray-900 dark:text-white">
                    <div class="font-medium">{{ getVariantName(item.variant_id) }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">ID: {{ item.variant_id }}</div>
                  </div>
  ...
  const getVariantName = (variantId) => {
    return variantsMap.value[variantId] || `Загрузка...`
  }
  ...
  const loadVariantsNames = async (variantIds) => {
    ...
    try {
      ...
      variantIds.forEach(id => {
         if (!variantsMap.value[id]) {
            variantsMap.value[id] = `Вариант ${id}`
         }
      })
    }
  }
  ...
  const handleIssue = async () => {
    ...
    try {
      ...
      let errorMessage = err.response?.data?.detail || 'Ошибка при выдаче товаров'
      
      // Replace "Товар ID: 123" with the actual variant name from our map
      if (typeof errorMessage === 'string') {
        errorMessage = errorMessage.replace(/Товар ID: (\d+)/g, (match, id) => {
          return variantsMap.value[id] ? `"${variantsMap.value[id]}"` : match
        })
      }
  ...
  const initModal = () => {
    ...
    if (props.items && props.items.length) {
      ...
      // Load variant names
      const variantIds = props.items.map(item => item.variant_id)
      loadVariantsNames(variantIds)
    }
    loadWarehouses()
  }
